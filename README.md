...
- 🔒 **文件完整性验证** - 移动文件时实现三重验证（大小、内容长度、MD5校验和）确保数据安全
...
全局安装后，可在任何目录下使用 `file-cleanup` 命令：
...
5. **文件内容完整性验证** - 确保文件移动过程中内容完整
   - 三重验证机制：文件大小比对、内容长度验证、MD5校验和比对
   - 复制失败时自动清理不完整目标文件，保留原始文件
   - 验证通过后才删除源文件，确保数据安全
