...
一个基于 Node.js 的命令行文件清理工具，具有智能回收站功能，可按扩展名、保留天数批量移动指定文件夹及其子目录中的旧文件到自定义回收站目录，或根据需要直接删除符合条件的文件，提供配置管理、系统保护与详细日志记录。
...
- 🗑️ **直接删除功能** - 可直接删除符合条件的文件，不执行移动和压缩操作
- ♻️ **回收站功能** - 自动将清理的文件移动到自定义回收站目录
- ⚙️ **回收站自定义设置** - 可配置回收站目录路径
- 📋 **目录结构保留** - 移动到回收站目录时保留原始文件结构和属性
...
2. **-f, --force 参数** - 可以与需要确认的操作参数搭配使用
   - 与 `--clear` 搭配：跳过所有确认提示，并直接删除符合条件的文件，不执行移动操作
     - 此模式下文件不会被移动到回收站，而是直接从磁盘删除
     - 直接删除的文件不可恢复，请谨慎使用
   - 与 `--configclear` 搭配：跳过清空配置的确认提示
   - 使用此参数时请格外谨慎，因为它会跳过所有安全确认步骤，并且直接删除的文件不可恢复
...
5. **通配符清理（跳过确认）** - 当配置文件中allowedExtensions设置为"*"（必须使用引号包裹）且需要跳过确认提示时
   ```bash
   file-cleanup --clear --force
   ```

6. **跳过确认的配置清空** - 当您需要清空所有文件夹配置且无需确认时
   ```bash
   file-cleanup --configclear --force
   ```

7. **配置管理** - 当您仅需管理清理配置（添加、删除、修改文件夹）时
   ```bash
